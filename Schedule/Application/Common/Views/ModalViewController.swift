...
//  schedule
//
//  Created by vladislav on 11.10.2020.
//  Copyright © 2020 mac. All rights reserved.
//

import UIKit

// MARK: - InteractiveModalTransitioningDelegate
...
// MARK: - ModalPresentationController
...
final class ModalPresentationController: UIPresentationController {
    ...
    private let presentedYOffset: CGFloat = 296
    ...
    @objc func didPan(pan: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            switch state {
            case .interaction:
                presented.frame.origin.y = location.y + (presentedView?.frame.height)!
            case .presentation:
                presented.frame.origin.y = location.y
            }
            direction = velocity.y
        case .ended:
            let maxPresentedY = container.frame.height - (presentedView?.frame.height)!
    ...
    override func containerViewWillLayoutSubviews() {
        ...
        self.presentedView?.leadingAnchor.constraint(equalTo: containerView!.leadingAnchor, constant: 15.0).isActive = true
        self.presentedView?.trailingAnchor.constraint(equalTo: containerView!.trailingAnchor, constant: -15.0).isActive = true
        self.presentedView?.bottomAnchor.constraint(equalTo: containerView!.bottomAnchor, constant: -15.0).isActive = true
        self.presentedView?.heightAnchor.constraint(equalToConstant: 270).isActive = true
    }
